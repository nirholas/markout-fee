...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(MarkoutFeeHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new MarkoutFeeHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, MarkoutFeeHook.Config({baseFee: 500, maxSurcharge: 9_500, alphaWad: 0.5e18, minHorizon: 60, saturationTicks: 50}));
