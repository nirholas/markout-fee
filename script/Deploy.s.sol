...
/**
 * @title DeployMarkoutFee
 * @notice Deploys MarkoutFee deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployMarkoutFee is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(MarkoutFeeHook).creationCode;
        ...
        console2.log("MarkoutFee ", predicted);
        ...
        vm.startBroadcast();
        MarkoutFeeHook hook = new MarkoutFeeHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("MarkoutFee")),
