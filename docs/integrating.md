# Integrating MarkoutFee

## From Solidity

```solidity
import {MarkoutFeeHook} from "markout-fee/src/hooks/MarkoutFeeHook.sol";
...
hook.configure(key, MarkoutFeeHook.Config({ /* see the README */ }));
...
const address = hookAddress("markout-fee", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "MarkoutFee"
...
More at https://markout-fee.pages.dev.
